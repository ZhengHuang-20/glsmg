      ...
      <a-spin :spinning="confirmLoading">
        <a-form :form="form">
          ...
          <a-form-item
              ...
              :wrapperCol="wrapperCol"
              label="授权标识">
            <a-input placeholder="请输入授权标识, 如: user:list"
...
export default {
  ...
  computed: {
    validatorRules: function () {
      return {
        ...
        url: {rules: [{required: this.show, validator: this.checkUrl, message: '请输入菜单路径，已/开头!'}]},
  ...
  methods: {
    checkUrl(rule, value, cb) {
      let reg = /^(\/|http|\{).+/;
      if (!this.show || reg.test(value)) {
        return cb();
      }
      cb(new Error('请输入菜单路径，已/或http或{开头'));
    },
    loadTree() {
      var that = this;
    ...
    edit(record) {
      ...
      //----------------------------------------------------------------------------------------------
      this.appId = this.model.appId;
      this.visible = true;
      this.loadTree();
      let fieldsVal = pick(this.model, 'name', 'appId', 'perms', 'permsType', 'component', 'redirect', 'url', 'sortNo', 'menuType', 'status');
      this.$nextTick(() => {

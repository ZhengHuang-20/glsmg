  ...
  <a-card :bordered="false">
    ...
    <div>
      <a-tabs defaultActiveKey="1" @change="callback">
        ...
        <a-tab-pane tab="系统日志" key="3"></a-tab-pane>
      </a-tabs>
    </div>
    ...
    <a-table
      ...
      <span slot="logContent" slot-scope="text, record">
          <s-ellipsis :value="text" :length="40"/>
        </span>
    </a-table>
    <!-- table区域-end -->
  </a-card>
...
import SEllipsis from '@/components/sbc/SEllipsis'
...
export default {
  ...
  methods: {
    ...
    // 日志类型
    callback(key) {
      /*let clen = this.columns.length;
      // 动态添加操作类型列
      if (key == 2 && clen <= 9) {
        this.columns.splice(7, 0, this.operateColumn);
      } else if ((key == 1 || key == 3) && clen == 9) {

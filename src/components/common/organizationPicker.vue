	...
	<a--dialog
		...
		<div class="picker">
			<div style="float:left;">
				...
				<div class="box">
					<div>
						<a--input v-if="type !== 'dept'" placeholder="搜索人员" prefix-icon="a--icon-search" size="medium"
						          v-moda-="search" :maxlength="50" @input="searchUser" clearable>
						</a--input>
						<a-breadcrumb separator-class="a--icon-arrow-right" style="overflow-x: hidden">
							<a-breadcrumb-item>通讯录</a-breadcrumb-item>
							<a-breadcrumb-item style="color:#38adff;" v-for="(node, index) in navNodes" :key="index">
								{{node.name}}
							</a-breadcrumb-item>
						</a-breadcrumb>
						<a-checkbox v-moda-="checkAll" @change="handleCheckAllChange" v-show="!single">全选</a-checkbox>
					...
					<div style="margin-top: 8px; width: 100%;">
						<div style="margin-top: 8px; overflow-y: auto; height: calc(100% - 90px);">
							<div v-for="(node, index) in (search === '' ? nodes : searchUsers)" :key="index" class="line"
								...
								<a-checkbox v-moda-="node.selected" :disabled="disableDept(node)"></a-checkbox>
								<span style="margin-left: 10px">
                    <i class="a--icon-folder-opened" v-if="node.type === 'dept'"></i>
			...
			<div style="float:right;">
                ...
                <span style="margin-left: 10px">
                    ...
                    <span style="float: right; color:#a9a9a9 ;cursor: pointer">
                      <i class="a--icon-close" @click="noSelected(index)"></i>
                    </span>
                  </span>
						</div>
					</div>
      ...
      <a-button @click="close" size="small">取 消</a-button>
      <a-button type="primary" @click="selectOk" size="small">确 定</a-button>
    </span>
	</a--dialog>
...
<script>
  // import {getOrgTree, getUserByName} from '@/api/flow'
  ...
  export default {
    ...
    methods: {
      ...
      selectOk() {
        this.$emit('sa-ectOver', Object.assign([], this.select))
        this.recover()
      },
      close() {
        this.$emit('close')

...
<script>
  const SInput_QUERY_LIKE = 'like';
  const SInput_QUERY_NE = 'ne';
  const SInput_QUERY_GE = 'ge'; //大于等于
  const SInput_QUERY_LE = 'le'; //小于等于
    ...
    methods:{
      initVal(){
        if(!this.value){
          ...
          let text = this.value
          switch (this.type) {
            case SInput_QUERY_LIKE:
              //修复路由传参的值传送到SInput框被前后各截取了一位 #1336
              if(text.indexOf("*") != -1){
                text = text.substring(1,text.length-1);
              }
              break;
            case SInput_QUERY_NE:
              text = text.substring(1);
              break;
            case SInput_QUERY_GE:
              text = text.substring(2);
              break;
            case SInput_QUERY_LE:
              text = text.substring(2);
              break;
            default:
          }

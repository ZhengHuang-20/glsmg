<template>
  <div>
    <div style="display:flex;justify-content:flex-start;">
      ...
      <a-button type="primary" @click="downTemplate()" icon="download" style="position: absolute;
    left: 110px;
    top: 4px;">下载模板</a-button>
    </div>
...
export default {
  ...
  created() {
    let headers = {};
    headers[`${process.env.VUE_APP_ACCESS_TOKEN}`] = Vue.ls.get(CACHE_KEY_ACCESS_TOKEN);
    this.headers = headers;
  },
  methods: {
    downTemplate(){
      window.open("https://sbcshop.oss-cn-beijing.aliyuncs.com/22/12/28/947c5fb37fd44b12aa328778.xlsx");
    },
    initFileList(paths) {
    ...
    beforeUpload: function (file) {
      ...
      if (fileName.indexOf('.xls') < 0) {
        if (fileName.indexOf('.xlsx') < 0) {
          this.$message.warning('请上传excel形式的文件');
          return false;
        }
      }
    },
    handleChange(info) {
      ...
      if (fileList.length > 1) {
        this.$message.warning('仅支持上传一个文件!');
        return;
      }

...
function loadRoutes(routesConfig) {
    ...
    if (asyncRoutes) {
        if (routesConfig && routesConfig.length > 0) {
            ...
            const routes = generateRsyncRouter(routesConfig);
...
function deepMergeRoutes(target, source) {
    ...
    const mapRoutes = routes => {
        routes.forEach(item => {
            ...
            item.children && mapRoutes(item.children)
        })
        return routesMap
    }
...
// 生成首页路由
export function generateRsyncRouter(data) {
    let indexRouter = [{
        path: '/',
        component: () => import('@/layouts/tabs'),
        //component: resolve => require(['@/layouts/tabs/TabsView.vue'], resolve),
        meta: {title: '首页'},
        redirect: '/dashboard',
        children: [
...
function generateChildRouters(data) {
    ...
    for (let item of data) {
        let componentPath;
        console.log(item)
        let component = item.component.replace(/^\//, '');
        let _component = component;
        if (component.indexOf("layouts") >= 0) {
            component = component.replace(/^layouts\//, '') + '.vue';
            // ### require一定要是对应项目路径！！！要不在打包时，webpack无法找到对应代码路径
            componentPath = resolve => require(['@/layouts/' + component], resolve)
        } else {
            component = component + '.vue';
            // ### require一定要是对应项目路径！！！要不在打包时，webpack无法找到对应代码路径
            componentPath = resolve => require(['@/views/' + component], resolve)
        }


        let meta = item.meta || {};
        // eslint-disable-next-line
        let URL = (meta.url || '').replace(/{{([^}}]+)?}}/g, (s1, s2) => eval(s2)) // URL支持{{ window.xxx }}占位符变量
        if (isURL(URL)) {
            meta.url = URL;
        }
        let menu = {
            ...
            meta: {
                title: meta.title || '',
                authority: item.authority || meta.authority || '*',
                icon: item.icon || meta.icon,
                page: item.page || meta.page,
                link: item.link || meta.link,
                url: meta.url/*||item.path*/,

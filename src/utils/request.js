...
import {CACHE_KEY_ACCESS_TOKEN, TENANT_ID} from "@/store/mutation-types"

/**
 * 【指定 axios的 baseURL】
 * 如果手工指定 baseURL: '/sbc-boot'
...
const err = (error) => {
    if (error.response) {
        ...
        switch (error.response.status) {
            case 403:
                notification.error({message: '系统提示', description: '请求资源未被授权', duration: 4})
                break
            case 500:
...
service.interceptors.request.use(config => {
    ...
    if (token) {
        config.headers[`${process.env.VUE_APP_ACCESS_TOKEN}`] = token // 让每个请求携带自定义 token 请根据实际情况自行修改
    }
